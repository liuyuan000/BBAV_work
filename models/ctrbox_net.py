...
class CTRBOX(nn.Module):
    def __init__(self, heads, pretrained, down_ratio, final_kernel, head_conv):
        ...
        # channels = np.array([3, 64, 256, 512, 1024, 2048])
        channels = [3, 64, 128, 256, 512, 1024, 2048]
        assert down_ratio in [2, 4, 8, 16]
        self.l1 = int(np.log2(down_ratio))
        self.width = 0.5
        # self.base_network = resnet.resnet101(pretrained=pretrained)
        self.base_network = YOLOPAFPN(depth=0.5, width=self.width)
        self.dec_c1 = CombinationModule(512*self.width, 256*self.width, group_norm=True)
        self.dec_c2 = CombinationModule(1024*self.width, 512*self.width, group_norm=True)
        self.dec_c3 = CombinationModule(2048*self.width, 1024*self.width, group_norm=True)
        # self.dec_c4 = CombinationModule(2048, 1024, batch_norm=True)
        self.heads = heads

        # self.conv1 = nn.Conv2d(int(128*self.width), int(256*self.width), kernel_size=3, stride=1, padding=1, groups=int(128*self.width), bias=False)
        # self.bn1 = nn.BatchNorm2d(int(256*self.width))
    ...
    def forward(self, x):
        ...
        '''
        c1_combine_con = self.dec_c3(x[-1], x[-2])
        c1_combine_con = self.dec_c2(c1_combine_con, x[-3])
        c1_combine_con = self.dec_c1(c1_combine_con, x[-4])
        # c1_combine_con = self.conv1(c1_combine_con)
        # out = c1_combine_con
        ...
        dec_dict = {}
        for head in self.heads:
            dec_dict[head] = self.__getattr__(head)(c1_combine_con) # torch.Size([8, 256, 152, 152])
